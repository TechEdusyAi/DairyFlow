...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/user/orders', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
  ...
  // Agent routes
  app.get('/api/agent/route', isAuthenticated, async (req: any, res) => {
    try {
  ...
  app.patch('/api/agent/delivery/:id/status', isAuthenticated, async (req: any, res) => {
    try {
      const { id } = req.params;
  ...
  app.patch('/api/agent/route-stop/:id/status', isAuthenticated, async (req: any, res) => {
    try {
      const { id } = req.params;
      const { status } = req.body;
      const deliveredAt = status === 'delivered' ? new Date() : undefined;
  ...
  // Admin routes
  const adminAuth = [isAuthenticated, (req: any, res: any, next: any) => {
  ...
  app.patch('/api/admin/inventory/:productId', adminAuth, async (req, res) => {
    try {
  ...
  app.post('/api/admin/agents/:userId/details', adminAuth, async (req, res) => {
    try {
  ...
  app.post('/api/admin/optimize-routes', adminAuth, async (req, res) => {
    try {
